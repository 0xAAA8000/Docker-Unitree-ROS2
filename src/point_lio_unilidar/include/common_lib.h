...
/*** ROS 2 does not provide Time::toSec()/fromSec(); these two helpers keep the
     original algorithm code working with plain double timestamps. ***/
...
inline rclcpp::Time get_ros_time(double timestamp)
{
    ...
    auto nanosec = static_cast<uint32_t>((timestamp - sec) * 1e9);
