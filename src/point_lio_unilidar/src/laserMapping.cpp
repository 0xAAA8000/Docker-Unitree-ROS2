...
{
    flg_exit = true;
    RCLCPP_WARN(rclcpp::get_logger("point_lio"), "catch sig %d", sig);
...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("point_lio"), "lidar loop back, clear buffer");
...
// void livox_pcl_cbk(const livox_ros_driver::CustomMsg::ConstSharedPtr &msg)
// {
...
void imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("point_lio"), "imu loop back, clear deque");
...
int main(int argc, char **argv)
{
    ...
    // The Livox AVIA custom-message path of the original package needs
    // livox_ros_driver2, which is not part of this image; only the standard
    // sensor_msgs/PointCloud2 path is wired up.
    ...
    auto sub_imu = nh->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic, rclcpp::SensorDataQoS().keep_last(2000), imu_cbk);

    auto pubLaserCloudFullRes = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_registered", 20);

    auto pubLaserCloudFullRes_body = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_registered_body", 20);

    auto pubLaserCloudEffect = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/cloud_effected", 20);

    auto pubLaserCloudMap = nh->create_publisher<sensor_msgs::msg::PointCloud2>("/pointlio/laser_map", 20);

    auto pubOdomAftMapped = nh->create_publisher<nav_msgs::msg::Odometry>("/pointlio/odom", 20);

    auto pubPath = nh->create_publisher<nav_msgs::msg::Path>("/pointlio/path", 20);

    auto plane_pub = nh->create_publisher<visualization_msgs::msg::Marker>("/pointlio/planner_normal", 20);
    ...
    while (rclcpp::ok())
    {
        ...
        if (flg_reset)
        {
            RCLCPP_WARN(rclcpp::get_logger("point_lio"), "reset when rosbag play back");
            p_imu->Reset();
            flg_reset = false;
            continue;
        }
        ...
        if (!use_imu_as_input)
        {
            ...
            for (k = 0; k < time_seq.size(); k++)
            {
                ...
                {
                    RCLCPP_WARN(rclcpp::get_logger("point_lio"), "No point, skip this scan!\n");
                    idx += time_seq[k];
                    continue;
                }
